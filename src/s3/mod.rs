...
//! becomes the bottleneck long before the parser does. Measured in-region on
//! EC2, one connection sustained ~45 MiB/s against ~76 MiB/s for an 8-way
//! window on the same object. This module fetches an object as a sliding
//! window of concurrent byte ranges, reassembled in order, presented as a
//! plain [`std::io::Read`].
//!
//! [`range`] holds the store-agnostic fetch window; the S3 specifics live
//! behind [`S3Reader`] and its builder.
